...
**Who may answer.** The token is a correlator, not a password: it travels in plaintext into the channel, so every member can read it. Both paths re-check that the answering user is in the alias's [peer group](./peer-groups.md), the same authority that decides whose messages the agent will act on. An unauthorized reply or tap is logged at `WARN` and ignored, and the prompt stays open so an authorized operator can still answer it.
